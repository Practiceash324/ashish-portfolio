...
const DEFAULT_ABOUT_PHOTO = SITE_IMAGES.aboutProfile;
const DEFAULT_WORKSPACE_PHOTO = SITE_IMAGES.classroomWorkspace;
const ABOUT_STORAGE_KEY = 'ashish_portfolio_about_photo_v1';
const WORKSPACE_STORAGE_KEY = 'ashish_portfolio_class_photo_v1';
...
export const AboutPage: React.FC<AboutPageProps> = ({ onNavigate, onOpenContact }) => {
  const [profilePhoto, setProfilePhoto] = useState<string>(DEFAULT_ABOUT_PHOTO);
  const [workspacePhoto, setWorkspacePhoto] = useState<string>(DEFAULT_WORKSPACE_PHOTO);
  ...
  useEffect(() => {
    try {
      const savedProfile = localStorage.getItem(ABOUT_STORAGE_KEY);
      if (savedProfile) setProfilePhoto(savedProfile);

      const savedWorkspace = localStorage.getItem(WORKSPACE_STORAGE_KEY);
      if (savedWorkspace) setWorkspacePhoto(savedWorkspace);
    } catch {
      // Fallback
    }
  }, []);
  ...
  const handleUpdateProfilePhoto = (url: string) => {
    setProfilePhoto(url);
    try {
      localStorage.setItem(ABOUT_STORAGE_KEY, url);
    } catch {
      // ignore
    }
  };
  ...
  const handleResetProfilePhoto = () => {
    setProfilePhoto(DEFAULT_ABOUT_PHOTO);
    try {
  ...
  const handleUpdateWorkspacePhoto = (url: string) => {
    setWorkspacePhoto(url);
    try {
      localStorage.setItem(WORKSPACE_STORAGE_KEY, url);
    } catch {
      // ignore
    }
  };

  const handleResetWorkspacePhoto = () => {
    setWorkspacePhoto(DEFAULT_WORKSPACE_PHOTO);
    try {
  ...
  return (
    <div className="pt-24 pb-20 bg-[#0B0B14] min-h-screen text-slate-100">
            ...
            <div className="relative group">
              ...
              <div className="relative rounded-3xl bg-[#10101C] p-3.5 border border-white/15 shadow-2xl overflow-hidden">
                <div className="relative aspect-[4/5] rounded-2xl overflow-hidden bg-slate-900">
                  ...
                  {/* Quick Photo Swap Trigger */}
                  <div className="absolute top-3.5 right-3.5">
                    <button
                      onClick={() => setIsProfileModalOpen(true)}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-black/80 backdrop-blur-md border border-white/20 text-white text-xs font-semibold hover:bg-violet-600 hover:border-violet-400 transition-all shadow-xl cursor-pointer"
                ...
                <div className="relative aspect-[16/10] rounded-2xl overflow-hidden bg-slate-900">
                  ...
                  <div className="absolute top-3 right-3">
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-black/80 backdrop-blur-md border border-white/20 text-slate-300 hover:text-white hover:bg-violet-600 transition-all cursor-pointer"
      ...
      {/* Modals for Photo Customization */}
      <PhotoSwapModal
        isOpen={isProfileModalOpen}
        onClose={() => setIsProfileModalOpen(false)}
        currentPhoto={profilePhoto}
        onSavePhoto={handleUpdateProfilePhoto}
        onResetPhoto={handleResetProfilePhoto}
      />

      <PhotoSwapModal
        isOpen={isWorkspaceModalOpen}
        onClose={() => setIsWorkspaceModalOpen(false)}
        currentPhoto={workspacePhoto}
        onSavePhoto={handleUpdateWorkspacePhoto}
        onResetPhoto={handleResetWorkspacePhoto}
      />
    </div>
  );
};
