...
const DEFAULT_CONTACT_PHOTO = 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=1000&auto=format&fit=crop';
const CONTACT_PHOTO_KEY = 'ashish_portfolio_contact_photo_v1';
...
export const ContactPage: React.FC<ContactPageProps> = ({ 
  ...
  const [photoUrl, setPhotoUrl] = useState<string>(DEFAULT_CONTACT_PHOTO);
  const [isPhotoModalOpen, setIsPhotoModalOpen] = useState(false);

  useEffect(() => {
    try {
      const saved = localStorage.getItem(CONTACT_PHOTO_KEY);
  ...
  const handleUpdatePhoto = (newUrl: string) => {
    setPhotoUrl(newUrl);
    try {
      localStorage.setItem(CONTACT_PHOTO_KEY, newUrl);
    } catch {
      // ignore
    }
  };
  ...
  const handleResetPhoto = () => {
    setPhotoUrl(DEFAULT_CONTACT_PHOTO);
    try {
      localStorage.removeItem(CONTACT_PHOTO_KEY);
    } catch {
      // ignore
    }
  };
  ...
  return (
    <div className="pt-24 pb-20 bg-[#0B0B14] min-h-screen text-slate-100">
                ...
                <div className="relative aspect-[4/4.5] sm:aspect-[4/5] rounded-2xl overflow-hidden bg-slate-900">
                  ...
                  <div className="absolute top-3.5 right-3.5">
                    <button
                      ...
                      title="Upload or change contact photo"
                    >
      ...
      {/* Photo Swap Modal for Contact Page */}
      <PhotoSwapModal
