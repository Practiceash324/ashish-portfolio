...
const DEFAULT_THANK_YOU_PHOTO = 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=1000&auto=format&fit=crop';
const THANK_YOU_PHOTO_KEY = 'ashish_portfolio_thankyou_photo_v1';
...
export const ThankYouPage: React.FC<ThankYouPageProps> = ({
  ...
  const [photoUrl, setPhotoUrl] = useState<string>(DEFAULT_THANK_YOU_PHOTO);
  const [isPhotoModalOpen, setIsPhotoModalOpen] = useState(false);

  useEffect(() => {
    try {
      const saved = localStorage.getItem(THANK_YOU_PHOTO_KEY);
      if (saved) {
        setPhotoUrl(saved);
      }
    } catch {
  ...
  const handleUpdatePhoto = (newUrl: string) => {
    setPhotoUrl(newUrl);
    try {
      localStorage.setItem(THANK_YOU_PHOTO_KEY, newUrl);
    } catch {
      // ignore
    }
  };
  ...
  const handleResetPhoto = () => {
    setPhotoUrl(DEFAULT_THANK_YOU_PHOTO);
    try {
      localStorage.removeItem(THANK_YOU_PHOTO_KEY);
    } catch {
      // ignore
    }
  };
  ...
  return (
    <div className="pt-24 pb-20 bg-[#0B0B14] min-h-screen text-slate-100">
          ...
          <div className="lg:col-span-5 relative group">
            ...
            <div className="relative rounded-3xl bg-[#10101C] p-4 border border-white/15 shadow-2xl space-y-4 h-full flex flex-col justify-between">
              ...
              <div className="relative aspect-[4/4.5] sm:aspect-[4/5] rounded-2xl overflow-hidden bg-slate-900">
                ...
                {/* Photo Upload / Swap Trigger */}
                <div className="absolute top-3.5 right-3.5">
                  <button
                    onClick={() => setIsPhotoModalOpen(true)}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-black/85 backdrop-blur-md border border-white/20 text-white text-xs font-semibold hover:bg-violet-600 hover:border-violet-400 transition-all shadow-xl cursor-pointer"
      ...
      {/* Thank You Page Photo Upload Modal */}
      <PhotoSwapModal
