...
const DEFAULT_HERO_PHOTO = SITE_IMAGES.heroPortrait;
const STORAGE_KEY = 'ashish_portfolio_hero_photo_v2';
...
export const HeroSection: React.FC<HeroSectionProps> = ({ 
  ...
  const [photoUrl, setPhotoUrl] = useState<string>(DEFAULT_HERO_PHOTO);
  ...
  const handleResetPhoto = () => {
    setPhotoUrl(DEFAULT_HERO_PHOTO);
    try {
  ...
  return (
    <section id="hero" className="relative min-h-[88vh] flex items-center pt-24 pb-16 md:pt-28 md:pb-20 overflow-hidden bg-[#0B0B14] text-slate-100">
            ...
            <div className="relative w-full max-w-md">
              ...
              <div className="relative rounded-3xl bg-[#10101C] border border-white/15 p-3.5 shadow-2xl overflow-hidden backdrop-blur-xl">
                ...
                <div className="relative aspect-[4/5] rounded-2xl overflow-hidden bg-slate-900 border border-white/10 group">
                  ...
                  {/* Interactive Photo Swap Trigger Button */}
                  <div className="absolute top-3.5 right-3.5">
                    <button
                      onClick={() => setIsPhotoModalOpen(true)}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-black/80 backdrop-blur-md border border-white/20 text-slate-200 hover:text-white hover:bg-violet-600 hover:border-violet-400 transition-all shadow-xl cursor-pointer text-xs font-semibold"
      ...
      {/* Photo Swap Modal Dialog */}
      <PhotoSwapModal
        isOpen={isPhotoModalOpen}
        onClose={() => setIsPhotoModalOpen(false)}
        currentPhoto={photoUrl}
        onPhotoChange={handlePhotoUpdate}
        onSavePhoto={handlePhotoUpdate}
